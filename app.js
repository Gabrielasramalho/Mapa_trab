...
//criar função dentro de uma const para checar o svg
const checarSvg = (image) => { 
    //por ser uma 
              fetch(image.src)
         .then((response) => response.text())
        .then((response) => { const span = Document.createElement('span'); 
       span.innerHTML = response;
       const inlineSvg =span.getElementByTagName('svg')[0];
       image.parentNode.replaceChild(inlineSvg);
       return true;
...
const getActions = () => {
    ...
    for(let i = 0; i < patrimonios. length; i++){
        patrimonios[i].onClick = () => { stateClicked(patrimonios[i]);}
    }
    getPatrimonios();
};
