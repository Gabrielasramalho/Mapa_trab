...
let dadosGerais = [
  ...
  },
  {
    ...
    modificaçõesSignificativasNosLimites:
      "Parque Nacional de Anavilhanas, Parque Nacional do Amanã e Reserva de Desenvolvimento Sustentável Mamirauá  em 2003.",
  ...
  {
    local:
      "Missões Jesuítas dos Guaranis: San Ignacio Mini, Santa Ana, Nuestra Señora de Loreto e Santa Maria Mayor (Argentina), Ruínas de São Miguel das Missões (Brasil) *",
    regiao: "Sul ",
    categoria: "Patrimônio Cultural",
    tamanho: "265,09 ha",
    zonaTampão: "Sem registro",
...
// declarar os dados do json q serão substituidos no html p/ gerar as descrições

//linkar os pontos do mapa aos dados do Json

//criar função para chamar os patrimonios que eu tenho no json em uma lista vazia
const patrimonios = [];


   //função de checar na minha pasta o svg e inserir ele na div no HTML
  ...
  const getActions = () => {
  const locais = document.getElementsByClassName("patrimonios");
  //? o q significa <
  for (let i = 0; i < patrimonios.length; i++) {
    patrimonios[i].onClick = () => {
      stateClicked(patrimonios[i]);
    };
  }
  getPatrimonios();
};
...
const stateClicked = (patriClicado) => {
  const code = patriClicado.getAtribute("code");
  
  const pt = patrimonios.find((patrimonio) => patrimonio.code == code);
  if (!pt) return;
  fillContent(pt);
};

const fillContent = (pt) => {
  const outputTitulo = document.getElementById(descricaoTitulo);
  const outputSubtitulo = document.getElementById(descricaoSubtitulo);
  const outputTexto = document.getElementById(descricaoTexto);

  outputTitulo.innerText = pt.local;
  outputSubtitulo.innerText = pt.estado;
  outputTexto.innerText = pt.regiao;
};
